...
echo ""
echo "El zip es el mismo para los dos ambientes: el backend no incrusta URLs al"
echo "empaquetar (a diferencia del frontend). El destino lo decide el .env que ya"
echo "vive en el servidor, y este zip no lo toca."
echo ""
echo "Siguiente paso: subir el zip a cPanel File Manager → carpeta del backend del ambiente:"
echo "  producción → ~/backend-v1.humae.com.mx/"
echo "  develop    → ~/develop.backend-v1.humae.com.mx/"
echo ""
echo "Luego corre en SSH:"
echo "  bash ~/deploy-backend.sh production"
echo "  bash ~/deploy-backend.sh develop"
